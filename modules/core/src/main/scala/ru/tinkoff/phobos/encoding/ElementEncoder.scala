...
trait ElementEncoder[A] { self =>
  def encodeAsElement(a: A, sw: XMLStreamWriter2, localName: String, namespaceUri: Option[String] = None): Unit

  def contramap[B](f: B => A): ElementEncoder[B] =
    new ElementEncoder[B] {
      def encodeAsElement(b: B, sw: XMLStreamWriter2, localName: String, namespaceUri: Option[String] = None): Unit =
